...
export interface ManuscriptEditing {
	...
	/**
	 * 밖에서 이 원고의 본문을 갈아 끼웠다 — 다시 읽어 앉힌다.
	 *
	 * 이력에서 되돌리면 서버의 본문이 바뀌는데, 에디터는 "이미 앉힌 원고는 다시
	 * 앉히지 않는다"고 정해 두어서 화면이 그대로 남는다. **저장을 일으키지 않고
	 * 다시 읽기만 한다** — 되돌린 것을 도로 덮어쓰면 안 되고, 저장이 돌면 완성이
	 * 또 강등된다.
	 */
	reload: () => void;
}
