 ...
 * id를 만든다. **cuid2다.**
 *
 * 전에는 `Math.random().toString(36).slice(2, 10)` 여덟 자였다. "한 브라우저
 * 안에서만 유일하면 된다"는 전제로 지은 것인데, **계정이 생기면서 그 전제가
 * 깨졌다** — 기기 여럿이 같은 보관함에 id를 던진다. 부딪히면 남의 원고를
 * 덮어쓰고, 그것을 피하려고 올릴 때마다 다시 매기는 코드가 따로 있었다.
 *
 * cuid2는 그 문제를 풀려고 만든 물건이다. 암호학적 난수에 기기 지문과 카운터를
 * 섞어 해시하므로, **서로 모르는 기기들이 각자 만들어도 부딪히지 않는다.**
 * 오프라인에서 브라우저가 스스로 id를 지으려면 이 성질이 있어야 한다.
 *
 * 길이는 열여섯 자로 줄여 쓴다(기본은 24자). **폴더 id가 경로 문자열에 박히기
 * 때문이다**(mpath) — `crypto.randomUUID()`를 쓰지 않는 이유도 그것이고, 세 겹만
 * 깊어져도 읽을 수 없는 길이가 된다. 열여섯 자면 `36^16 ≈ 8×10^24`라, 한 사람의
 * 보관함이 감당할 어떤 수에서도 부딪힐 일이 없다.
 *
 ...
 *
 * 옛 id는 여덟 자 난수라 "한 브라우저 안에서만" 유일했다. 다른 기기에서 쓴
 * 원고가 이미 계정에 있으면 부딪힐 수 있고, 그대로 올리면 남의 원고를 덮어쓴다.
 * 지금 만드는 id는 cuid2라 이 걱정이 없지만, **옛 보관함을 옮길 때는 그때 지은
 * 여덟 자짜리가 올라오므로** 이 길이 남아 있어야 한다.
 *
