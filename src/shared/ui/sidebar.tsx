...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			/*
			 * 정본은 여기서 쿠키에 상태를 적는다. 이 앱은 화면 설정을 모두
			 * localStorage에 두고 저장 실패를 배너로 알리므로, 저장은 이 컴포넌트를
			 * 쓰는 쪽(`open`/`onOpenChange`)이 맡는다.
			 */
		},
		[setOpenProp, open],
	);
	...
	const toggleSidebar = React.useCallback(() => {
		...
		// setOpenMobile은 setState라 늘 같은 함수다. 의존성에 둘 필요가 없다
	}, [isMobile, setOpen]);
...
function Sidebar({
	...
	return (
		<div
			// 경계는 useIsMobile과 같아야 한다. 어긋나면 그 구간에서 옆에 편 보관함과
			// 서랍이 한 번에 잡혀 첫 그림에 깜빡인다
