 ...
 *
 * 로컬 id는 `Math.random().toString(36).slice(2, 10)` 여덟 자라, 만든 주석대로
 * "한 브라우저 안에서만" 겹치지 않는다. 그것을 전역 열쇠로 쓰면 다른 사람과
 * 부딪히지만, 사용자 안으로 가두면 만들 때의 가정이 그대로 성립한다. 덕분에
 * 로컬 원고를 계정으로 올릴 때 id를 다시 매길 일이 대부분 없다 — 같은 계정에
 * 이미 그 id가 있을 때만 다시 매기면 된다.
 */
const owner = () =>
	text("user_id")
		.notNull()
